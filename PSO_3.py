# -*- coding: utf-8 -*-
"""
Created on Tue Apr  2 10:48:01 2019
...
class MyPSO(object):
    ...
    def Save(self,bestg,result):
        ...
        for i in range(np.shape(factorandrresult)[0]):
            if i == 4:
                file.write("   ")
                file.write(str(round(factorandrresult[i],3)))
                file.write(" ")
            else:
                file.write(str(round(factorandrresult[i],3)))
                file.write(" ")
        ...
        for iter in range(self.iter):
            ...
            for index, result in resultdict.items():
                if result > resultmax:
                    ...
                    pass
            if resultmax > 0.6870368177650102:
                print("Best result = " + str(resultmax))
                print("Best vector = " + str([round(a,3) for a in bestg.tolist()]))
